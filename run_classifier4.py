...
from model4 import BertForSequenceClassification, BertConfig, WEIGHTS_NAME, CONFIG_NAME
...
def main(i):
    ...
    ## Required parameters
    parser.add_argument("--data_dir", default='/home/MOSEI/Raw/text', type=str,
    ...
    parser.add_argument("--output_dir", default='CM-BERT_output4', type=str,
    ...
    ##############################################################################################################
    model = BertForSequenceClassification.from_pretrained(args.bert_model, cache_dir=cache_dir, num_labels = num_labels, q_size=args.q_size, alpha=args.alpha, contrastive=args.contrastive)
    ...
    {'params': [p for n, p in param_optimizer if "bert" not in n], 'lr': args.learning_rate, 'weight_decay': 0.01},
    ]
    ...
    train_audio,valid_audio,test_audio= pickle.load(open('/home/MOSEI/Raw/audio/paudio.pickle','rb'))

    train_audio_IS,valid_audio_IS,test_audio_IS= pickle.load(open('/home/MOSEI/Raw/audio/paudio_IS.pickle','rb'))

    train_video,valid_video,test_video= pickle.load(open('/home/MOSEI/Raw/video/pvideo.pickle','rb'))
    ...
    if args.do_test==True:
        ...
        model = BertForSequenceClassification.from_pretrained(args.bert_model, cache_dir=cache_dir, num_labels = num_labels, q_size=args.q_size, alpha=args.alpha,  contrastive=args.contrastive)
        model.load_state_dict(torch.load('CM-BERT_output4/pytorch_model.bin'))
        model.to(device)
        model.eval()
        ...
        predict_list1 = (predict_list[non_zeros] >= 0.5)
        truth_list1 = (truth_list[non_zeros] >= 0.5)
...
if __name__ == "__main__":
    ...
    acc7_list = []
    for i in range(1):
        os.system('mkdir CM-BERT_output4')
        results, batch_size, lr, alpha = main(i)
        ...
        os.system('rm -r CM-BERT_output4')
    ...
    acc_list.append(np.max(acc_array))
    F1_list.append(np.max(F1_array))
