...
class PgProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "ptrain.tsv")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "ptest.tsv")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "ptest.tsv")), "test")
...
def accuracy1(out, labels):
    num = 0
    for i in range(len(out)):
        if out[i]>=0.5 and labels[i]>=0.5:
            num = num+1
        else:
            if out[i]<0.5 and labels[i]<0.5:
                num = num+1
